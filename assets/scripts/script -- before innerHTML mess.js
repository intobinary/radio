...
Written for -- www.radio.intobinary.org
===*/
...
window.onload = function() {
	/*** OBJECTS AND VARIABLES ***/
		...
		var countChannels = 0, allChannels = "";
...
//		radio_playStation(channelOne);
	...
	/*** ACTIONS ***/
	document.querySelector("#category1station1 img").click(function() {
		alert("clicked");
	});
	/*** END ACTIONS ***/
			...
			request.onreadystatechange = function (){
				if (request.readyState == 4 && request.status == 200){
					...
					for (var i=0;i<station_data.length;i++) {
						...
						categoryString += '<li id="'+ categoryID +'" class="category app-content-menu-list-item"><h5>'+ category.name +'</h5><ul id="'+ categoryID +'subList" class="app-content-menu-subList">';
						
						
						for (var i1=0;i1<category.channels.length;i1++) {
							countChannels += 1;
							allChannels += category.channels[i1].url + ",";

							var stationID = "category" + (i+1) + "station" + (i1+1);
						
							subCategoryString = '<li id="'+ stationID +'" class="station app-content-menu-subList-item"><img src="assets/images/placeholder-1x1.png" class="test" data-channel="'+ category.channels[i1].url +'" /><h5>'+ category.channels[i1].name +'</h5></li>';
							
							categoryString += subCategoryString;
						}
						
						categoryString += "</ul></li>";
					}
					
					document.getElementById("channelList").innerHTML = categoryString;
				}
			}
			
			request.open("GET", "channels2.json");
			request.send();
			
            audioTag.addEventListener("volumechange", function (){
//                radio_setValue("playervolume", audioTag.volume);
            }, false);
            
//            audioTag.volume = radio_getValue("playervolume");
		}

		function radio_playStation(url) {
//			radio_playMP3(url);
			audioTag.src = url;
		}

		function radio_getValue(key) {
...
//			window.localStorage[key] = undefined;
		...
		function radio_setValue(key, value) {
//			window.localStorage[key] = "";
//			window.localStorage[key] = undefined;
